...
async def movie(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.effective_message.text[1:3] == 'm_':
        ...
        category = 'tv'
    req = requests.get(
        f'https://api.themoviedb.org/3/{category}/{id}?api_key=373ba86c0e94cd3f4edcdc6974e941e2&append_to_response=videos')
    req = req.json()
    if req:
        if category == 'movie':
...
async def next_prev(update: Update, context:ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query[-1:] == '1':
        keyboard = [
         [
            [InlineKeyboardButton(text='NEXT PAGE', callback_data='page' + '1')],
            [InlineKeyboardButton(text='PREV PAGE', callback_data='page' + '-1')],
         ]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.effective_message.reply_text(text='Not your search item?', reply_markup=reply_markup)
...
async def pagination(update:Update, context:ContextTypes.DEFAULT_TYPE)-> None:
    query = update.callback_query
    await query.answer()
    page, results = query.data
    items = results[page]
    link = items['id']
    try:
    ...
    if len(results) - page >= 1:
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.callback_query.message.reply_text(text=f'1/{len(results)}', reply_markup=reply_markup)
    ...
    if 'movie' in query.data.lower():
        ...
        if req:
            for items in req:
                ...
                # ]
                # reply_markup = InlineKeyboardMarkup(keyboard)
                # await update.callback_query.message.reply_text(text=f'1/{len(req)}', reply_markup=reply_markup)
        else:
            await update.callback_query.message.reply_text(
...
async def one(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data[0] == 'o':
        ...
        if link4:
            ...
            keyboard.append(
                [InlineKeyboardButton(text='Torrent downloader for andriod',
...
async def disclaimer(update, context):
    await update.message.reply_text(
        'NOTE:DOWNLOAD LINKS ARE NOT ACCURATE AND ARE NOT PART OF THE DATABASE THEY BELONG TO OTHER WEBSITES!\n WE OWE NO RESPONSIBILITY TO INAPPRORIATE CONTENTS!')


async def help(update, message):
    await update.message.reply_text(
        "If you click on a download link and it  takes you to a page with the title xxprox... just complete the rechapta by clicking 'i am not a robot' after the 'verify that you are a human text' and your download should start promptly ")
...
def main():
    ...
    # application.add_handler(CallbackQueryHandler(next_prev, pattern=str('page') + '+'))
    ...
    application.run_webhook(
        ...
        port=int(PORT),
        url_path=TOKEN,
        webhook_url='https://lit-hamlet-51436.herokuapp.com/' + TOKEN
    )
