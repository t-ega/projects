...
def search(query, category, results, page):
    if page:
        ...
        req = requests.get(f'https://api.themoviedb.org/3/search/{category}?api_key=373ba86c0e94cd3f4edcdc6974e941e2'
...
def hdtoday(title):
    title = title.replace(' ', '-')
    req = requests.get(f'https://hdtoday.tv/search/{title}')
    req = req.json()